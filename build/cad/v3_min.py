...
from anton_v3 import (camera_final, WALL_Y, CAM_TIP2HOLE, CAM_FLK_OUT,   # M-A3: Toms finale
                      CAM_FLK_T,                                          # [F1-Port 07-26] Fuß = Flanken-Ebene
...
RLID_SCREW = [(-RBOSS_X, RBOSS_Y), (RBOSS_X, -RBOSS_Y)]   # [F5-Port 07-26] 2 M3 DIAGONAL (Einheitsprinzip: MID 07-25)
...
def mount_antenna_ze(body):
    ...
    for sx, cy in AZE_SIDES:
        ...
        # (5) [F4 07-26, Port von MID 07-25] KAPPEN-SEITENHALTERUNG ERSATZLOS ENTFERNT
        #     (Ingenieur-Feedback Runde 2: Ohren-Bosse + horizontale Ø1,7-Piloten sparen,
        #     abgerundetes Design wie Kameraseite). Einheitsprinzip aller drei Sender.
    return body
...
if __name__ == "__main__":
    ...
    _blk_solids   = [Pos(px, py, (RBOSS_Z0+RBOSS_Z1)/2) * Cylinder(radius=RBOSS_D/2, height=RBOSS_Z1-RBOSS_Z0)
                     for (px, py) in RLID_SCREW]                  # 2 Eck-Bosse (F5: diagonal) (Dach-Deckel)
    _vtx = _vtx_env_solid()
    ...
    print(f"[boss]  2 Eck-Bosse (F5: diagonal) Ø{RBOSS_D} @{RLID_SCREW} Z{RBOSS_Z0:.0f}..{RBOSS_Z1:.0f} · Insert "
    ...
    print(f"[deckel-mount] 2 Eck-Bosse (F5: diagonal) @{RLID_SCREW} Z{RBOSS_Z0:.0f}..{RBOSS_Z1:.0f} · "
          f"Insert Ø{RBOSS_INS_D}×{RBOSS_INS_DP} Achse Z (Mündung {RBOSS_Z1:.0f}) → 2 M3 von OBEN")
    ...
    import numpy as _np
    _md = _tm.load(f"{OUTDIR}/skylive_V3_min_battery_door.stl")
